...
###################################################
file_name = "C:/Users/LA202059/Desktop/RHEA/rhea/resources/gasturbine/TP_L1/Datapacks/TP_L1_deck_RTO.txt"
data = pd.read_csv(file_name, sep="\t")
data.drop(['SHP','Throttle'],axis=1,inplace=True)
n_rows= data.shape[0]
...
for sample in samples:  #learning curve
    ...
    while x <n_times:
        ...
        for n in n_degree: #bias variance dilemma
            ...
            LR = LinearRegression() 
            x_train_test_pol=poly.fit_transform(df_train_test_sample.drop('PSFC',axis=1).values)

            
            scores=cross_val_score(LR,x_train_test_pol, df_train_test_sample.PSFC.values,cv=10)            
...
x_train_pol=poly.fit_transform(df_train_test.drop('PSFC',axis=1).values)
x_val_pol=poly.fit_transform(df_val.drop('PSFC',axis=1).values)
LR.fit(x_train_pol, df_train_test.PSFC.values)
PSFC_predicted=LR.predict(x_val_pol)
deltas=[]
for i in range(len(PSFC_predicted)):
    deltas.append((df_val.PSFC.values[i] -PSFC_predicted[i] )/df_val.PSFC.values[i]*100)
    
val_score=LR.score(x_val_pol, df_val.PSFC.values)
print('Validation score is:',val_score)

training_score= LR.score(x_train_pol, df_train_test.PSFC.values)
print('Training score is:',training_score)

df_val_results = df_val
df_val_results['PSFC_predicted']=PSFC_predicted
df_val_results['delta %']=(df_val_results.PSFC-df_val_results.PSFC_predicted)/df_val_results.PSFC*100
df_val_results.to_csv('validation.csv')

# save the model to disk
filename = 'TP_L1_RTO_PSFC.sav'
