...
class MassBreakdown_RHEA(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem("owe", OperatingWeightEmpty(self.hybrid,self.payload_from_npax,self.out_file), promotes=["*"])
...
class ComputePayloadfromOWE(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        OWE_no_seats =  inputs["data:weight:airframe:mass"] + inputs["data:weight:propulsion:mass"] + inputs["data:weight:systems:mass"] + inputs["data:weight:furniture:mass"] + inputs["data:weight:operational_no_seats:mass"] + inputs["data:weight:aircraft_empty:contingency"]   
        ...
        row_length = inputs["data:geometry:fuselage:PAX_length"]/(inputs["data:geometry:cabin:NPAX1"]/count_by_row)
        ...
        available_length = tailcone_length + row_length*int(max_pax/count_by_row -n_pax/count_by_row )
...
class OperatingWeightEmpty(om.Group):
    ...
    def setup(self):
        ...
        #Propulsion
        self.add_subsystem("turboprop_weight", TurbopropWeight(), promotes=["*"]) #to add once we do the sizing of the turboprop
        # if self.hybrid:
        self.add_subsystem("ATA28", FuelLinesWeight(), promotes=["*"])
        
        self.add_subsystem("hybrid_propulsion_weight", HybridPropulsionWeight(self.out_file), promotes=["*"])
