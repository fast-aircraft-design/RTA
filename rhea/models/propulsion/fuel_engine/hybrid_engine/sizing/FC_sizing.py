...
class FC_sizing(ExplicitComponent):
    ...
    def __init__(self):
        super().__init__()
        self.T_ref = 298.0  # Units['K']
        self.P_ref = 1.013  # Units['bar']
        self.M_H2 = 2.02e-3  # Units['kg/mol']
        self.hf_H2Ol = -285826.0  # Units['J/mol'] #  if you use this you get HHV
        self.hf_H2Og = -241826.0  # Units['J/mol'] #  if you use this you get LHV
        ...
        self.sf_H2 = (
            130.68  # Units['J/mol'] # H2 entropy of formation at standard state
        )
        self.sf_O2 = (
            205.14  # Units['J/mol'] # O2 entropy of formation at standard state
        )
        ...
        self.alpha = self.i_lA + (1 / self.T_A) * (self.i_lA - self.i_lB) / (
            1 / self.T_B - 1 / self.T_A
        )
    ...
    def setup(self):
        ...
        self.add_input("tuning:propulsion:electric_systems:fuel_cell:k_eta", 1.0)
        ...
        self.add_input("data:propulsion:electric_systems:fuel_cell:layout:rows", 2.0)
        ...
        self.add_input(
            "data:propulsion:electric_systems:fuel_cell:sizing:T_op", np.nan, units="K"
        )
        self.add_input(
            "data:propulsion:electric_systems:fuel_cell:sizing:P_op",
            np.nan,
            units="bar",
        )
        ...
        self.add_output(
            "data:propulsion:electric_systems:fuel_cell:sizing:nominal_operational_point:V_cell"
        )
        self.add_output(
            "data:propulsion:electric_systems:fuel_cell:sizing:nominal_operational_point:V_stack"
        )
        self.add_output(
            "data:propulsion:electric_systems:fuel_cell:sizing:nominal_operational_point:V_pack"
        )
        # self.add_output("data:propulsion:electric_systems:fuel_cell:sizing:nominal_operational_point:V_core")

        self.add_output(
            "data:propulsion:electric_systems:fuel_cell:sizing:nominal_operational_point:i_cell"
        )
        self.add_output(
            "data:propulsion:electric_systems:fuel_cell:sizing:nominal_operational_point:i_stack"
        )
        self.add_output(
    ...
    def compute(self, inputs, outputs):
        ...
        power_electronics_power_in = power_electronics.get_idc_power(
            power_electronics_power, power_flow="downstream"
        )

        P_sizing_fc = (
            power_electronics_power_in / Gross_net_power_ratio + Power_offtake
        )  # from gross power to net power
        ...
        i_0 = self.A * np.exp(
            -self.Ea / (const.R * T)
        )  # Units['A/cm**2'] # Exchange current density
        ...
        ########## LHV ###########
        Delta_S = self.sf_H2Og - (
            self.sf_H2 + 0.5 * self.sf_O2
        )  # Units['J/K'] # Entropy change
        Delta_H = self.hf_H2Og - (
            self.hf_H2 + 0.5 * self.hf_O2
        )  # Units['J']   # Enthalpy change

        Delta_G = Delta_H - self.T_ref * Delta_S  # Units['J']   # Gibbs free energy

        E_r = -Delta_G / (
            2.0 * self.F
        )  # Units['V'] # Reversible potential at standard conditions
        delta_E_r_T = (
            Delta_S / (2.0 * self.F) * (T - self.T_ref)
        )  # Units['V'] # Potential variation at non standard temperature
        delta_E_r_P_ref = 0
        delta_E_r_P = (
            self.C * np.log(P / self.P_ref) + delta_E_r_P_ref
        )  # Units['V'] # Potential variation at non standard pressure
        V_rev = (
            E_r + delta_E_r_T + delta_E_r_P
        )  # Units['V'] # Reversible potential at non standard conditions
        ...
        for i in i_vec:
            ...
            v_conc = (
                8.314 * T / (2 * self.F) * (1 + 1.0 / 0.5) * np.log10(i_l / (i_l - i))
