...
class Fuel_cell(object):
    def __init__(
        self, V_vec, i_vec, Nstacks, Npacks, Acell, Ncells, Nmotors
    ):  # , k_eta=1):
    ...
    def get_fc_perfo(self, P_elec):
        ...
        FC_H2_tot = (
            self.M_H2 * P_req / (2 * self.F * V_req_cell)
        )  # Units['kg/s']   # From reference[4] (eq. A2.8)
        air_flow_tot = (
            3.57 * (10 ** (-7)) * 2 * P_req / V_req_cell
        )  # Units['kg/s']   # From reference[4] (eq. A2.4)
