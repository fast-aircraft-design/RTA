...
class Thrust(object):
    ...
    def compute_design(self,atmosphere,M,bleed_offtake, eta_mech,etapolt_turb,pid_nozzle,etapolt_nozz):
...
#        cp_t=1120.
        ...
        while (abs(dT) > 20):        
            ...
            if (Pt_out/p0*pid_nozzle < 1.893):
                M_out_nozzle = np.sqrt(2./(gamma_t-1)*((Pt_out/p0*pid_nozzle)**((gamma_t-1)/gamma_t)-1))
                Tt_out = ht_out/cp_t
