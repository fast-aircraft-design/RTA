...
class TPEngine_L1(AbstractFuelPropulsion):
    ...
    def compute_flight_points(self, flight_points: Union[FlightPoint, pd.DataFrame]):
        ...
        (
            ...
            engine_charac,
            thermo_data,
        ) = self.compute_power(atmosphere, mach, phase, self.T4)
        ...
        (
            ...
            engine_charac,
            thermo_data,
        ) = self.compute_engine_point(atmosphere, mach, phase, out_power)
        if abs(design_power - out_power) > 10000:
            #     # if phase.value==2:
            #     #     a=0
            #     # print(design_power/constants.hp)
            print(
                "Warning: compute engine point convergence",
                phase,
                altitude,
                design_power - out_power,
                "Watt",
            )
    ...
    def compute_engine_point(
        ...
        def compute_point(throttle):
            (
                ...
                engine_charac,
                thermo_data,
            ) = self.compute_power(atmosphere, mach, phase, throttle * self.T4)
        ...
        # throttle = sol.root
        throttle, infodict, ier, msg = fsolve(
            compute_point, x0=0.9, full_output=True, xtol=tol
        )
    ...
    def compute_power(
        ...
        if phase == 2:  #'MCL'
            ...
            Shp_gb_limit = (
                self.k_gb_MCL * self.RTO_power
            )  # 2001582.729865489#self.RTO_power #

        elif phase == 4:  #'FID'
            rating = self.k_th_FID
            Shp_gb_limit = (
                self.k_gb_FID * self.RTO_power
            )  # 2001582.729865489#self.RTO_power
        elif phase == 1:  #'TO'
            rating = self.k_th_NTO
            Shp_gb_limit = (
                self.k_gb_NTO * self.RTO_power
            )  # 2001582.729865489#self.RTO_power

        elif phase == 9:
            rating = self.k_th_MCT
            Shp_gb_limit = (
                self.k_gb_MCT * self.RTO_power
            )  # 2001582.729865489# self.RTO_power

        elif phase == 3:  #'CRZ'
            rating = self.k_th_MCR
            Shp_gb_limit = (
                self.k_gb_MCR * self.RTO_power
            )  # 2001582.729865489#self.RTO_power
            self.LP_bleed = 0.96
        elif phase == 8:  #'RTO'
            rating = self.k_th_RTO
            Shp_gb_limit = (
                self.k_gb_RTO * self.RTO_power
        ...
        hpc.temperature_ratio = 1 + self.k1 * (
            tau_b / (ram.temperature_ratio * lpc.temperature_ratio)
        )
        ...
        lpt.bleed_offtake = (
            self.HP_bleed * self.LP_bleed
        )  # =1 solo per verifica ratings
        ...
        pt.pi_t = (
            lpt.stagnation_pressure_out
            / lpt.stagnation_pressure_in
            * hpt.stagnation_pressure_out
            / hpt.stagnation_pressure_in
        )
        pt.tau_t = (
            lpt.stagnation_temperature_out
            / lpt.stagnation_temperature_in
            * hpt.stagnation_temperature_out
            / hpt.stagnation_temperature_in
        )
        ...
        # compute the thrust
        thrust.compute_offdesign(
            atmosphere, mach, self.HP_bleed, self.LP_bleed, self.pt_eta_mech
        )
        ...
        thermal_efficiency = (
            pt.shaft_takeoff * air_flow + jet_power_adim * air_flow
        ) / thermal_power
        ...
        return (
            ...
            thermo_power,
            Shp_gb_limit,
            [fuel_flow, residual_thrust, thermal_efficiency, ESHP, air_flow],
            thermo_data,
        )
