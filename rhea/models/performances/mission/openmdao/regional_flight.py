...
class SizingFlight_RHEA(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("data:mission:sizing:taxi_out:duration", 360., units="s")
        self.add_input("data:mission:sizing:takeoff:duration", 60., units="s")
        self.add_input("data:mission:sizing:landing:duration", 120., units="s")
        self.add_input("data:mission:sizing:taxi_in:duration", 240., units="s")
        ...
        self.add_input("data:mission:sizing:takeoff:V_2", units="m/s",val=60.)
        self.add_input("data:mission:sizing:takeoff:TP_power_rate",val=1.)
        self.add_input("data:mission:sizing:takeoff:EM_power_rate",val=0.)

       
        self.add_input("data:mission:sizing:initial_climb:EM_power_rate", 0.)
        self.add_input("data:mission:sizing:initial_climb:TP_power_rate", 1.)
        
        
        self.add_input("data:mission:sizing:climb:thrust_rate", np.nan)
        self.add_input("data:mission:sizing:climb:EM_power_rate", 0.,shape=POWER_RATE_COUNT)
        self.add_input("data:mission:sizing:climb:TP_power_rate", 1.,shape=POWER_RATE_COUNT)
        
        self.add_input("data:mission:sizing:diversion:climb:EM_power_rate", 0.)
        self.add_input("data:mission:sizing:diversion:climb:TP_power_rate",1.)        
        ...
        self.add_input("data:mission:sizing:cruise:EM_power_rate", 0.)
        self.add_input("data:mission:sizing:cruise:TP_power_rate", 1.)
        
        
        
        self.add_input("data:mission:sizing:diversion:cruise:EM_power_rate", 0.)
        self.add_input("data:mission:sizing:diversion:cruise:TP_power_rate", 1.)  
        ...
        self.add_input("data:mission:sizing:diversion:H2_allowances", 0.)
        ...
        self.add_input("data:mission:sizing:holding:EM_power_rate", 0.)
        self.add_input("data:mission:sizing:holding:TP_power_rate", 1.)  
        ...
        self.add_input("data:mission:sizing:landing:H2", 0., units="kg")
        self.add_input("data:mission:sizing:taxi_in:H2", 0., units="kg")
        ...
        self.add_input("data:mission:DOC:taxi_out:H2", 0., units="kg")
        ...
        self.add_input("data:mission:DOC:diversion:H2_allowances", 0.)
        ...
        self.add_input("data:mission:DOC:landing:H2", 0., units="kg")
        self.add_input("data:mission:DOC:taxi_in:H2", 0., units="kg")
        ...
        self.add_output("data:mission:sizing:diversion:climb:operational_ceiling", 0,units="m")
        ...
        self.add_output("data:propulsion:electric_systems:fuel_cell:H2_flow_rate", units="kg/s",val=0)
        ...
        self.add_output("data:mission:sizing:main_route:climb:H2", units="kg" ,val=0.)
        self.add_output("data:mission:sizing:main_route:cruise:H2", units="kg", val=0.)
        self.add_output("data:mission:sizing:main_route:descent:H2", units="kg", val=0.)
        ...
        self.add_output("data:mission:sizing:diversion:climb:fuel", units="kg",val=0.)
        self.add_output("data:mission:sizing:diversion:cruise:fuel", units="kg",val=0.)
        self.add_output("data:mission:sizing:diversion:descent:fuel", units="kg",val=0.)
        self.add_output("data:mission:sizing:diversion:climb:H2", units="kg" ,val=0.)
        self.add_output("data:mission:sizing:diversion:cruise:H2", units="kg" ,val=0.)
        self.add_output("data:mission:sizing:diversion:descent:H2", units="kg", val=0.)
        
        self.add_output("data:mission:sizing:diversion:climb:distance", units="m",val=0.)
        self.add_output("data:mission:sizing:diversion:cruise:distance", units="m",val=0.)
        self.add_output("data:mission:sizing:diversion:descent:distance", units="m",val=0.)

        self.add_output("data:mission:sizing:diversion:climb:duration", units="s",val=0.)
        self.add_output("data:mission:sizing:diversion:cruise:duration", units="s",val=0.)
        self.add_output("data:mission:sizing:diversion:descent:duration", units="s",val=0.)

        self.add_output("data:mission:sizing:holding:fuel", units="kg")
        self.add_output("data:mission:sizing:holding:H2", units="kg", val=0.)

        self.add_output("data:mission:sizing:ZFW", units="kg")
        self.add_output("data:mission:sizing:fuel", units="kg")
        self.add_output("data:mission:sizing:H2", units="kg", val=0.)
        self.add_output("data:mission:sizing:BAT_energy", units="J", val=0.)
        self.add_output("data:mission:sizing:trip_fuel", units="kg")
        self.add_output("data:mission:sizing:trip_H2", units="kg", val=0.)
    ...
    def compute_mission(self, inputs, outputs,loop_inputs,mission_type):
        ...
        electric_power_rates = {
            FlightPhase.INITIAL_CLIMB: inputs["data:mission:"+mission_type+":initial_climb:EM_power_rate"],
            FlightPhase.CLIMB: inputs["data:mission:"+mission_type+":climb:EM_power_rate"],
            FlightPhase.CRUISE: inputs["data:mission:"+mission_type+":cruise:EM_power_rate"],
        }     
        
        turbine_power_rates = {
            FlightPhase.INITIAL_CLIMB: inputs["data:mission:"+mission_type+":initial_climb:TP_power_rate"],
            FlightPhase.CLIMB: inputs["data:mission:"+mission_type+":climb:TP_power_rate"],
            FlightPhase.CRUISE: inputs["data:mission:"+mission_type+":cruise:TP_power_rate"],
        }         
        high_speed_polar = Polar(
            inputs["data:aerodynamics:aircraft:cruise:CL"],
            inputs["data:aerodynamics:aircraft:cruise:CD"],
        ...
        print('start main mission',TOW_input)#, int(propulsion_model.engine.Elec_nom_power),int(propulsion_model.engine.RTO_power) )
        ...
        end_of_takeoff = FlightPoint(
            mass=TOW_input-inputs["data:mission:"+mission_type+":taxi_out:fuel"]-inputs["data:mission:"+mission_type+":takeoff:fuel"],
        ...
        delta_time=[flight_points.time.iloc[1]-flight_points.time.iloc[0]] + [flight_points.time.iloc[i+1]-flight_points.time.iloc[i] for i in range(len(flight_points)-1)]
        flight_points['delta_time']=np.array(delta_time,dtype=object)
        fuel_delta_mass = flight_points.psfc * flight_points.TPshaft_power *flight_points.delta_time
        ...
        if 'FC' in self.options["propulsion_id"]:
            ...
            outputs["data:propulsion:electric_systems:fuel_cell:H2_flow_rate"] = max(flight_points.H2_fc.values)/propulsion_model.motor_count

        if 'H2' in self.options["propulsion_id"]:
            H2_delta_mass = flight_points.delta_time*flight_points.H2_fc
            flight_points['H2_mass']= H2_delta_mass.cumsum()
            outputs["data:propulsion:electric_systems:fuel_cell:H2_flow_rate"] = max(flight_points.H2_fc.values)/propulsion_model.engine_count
        ...
        top_of_climb = FlightPoint(flight_points[(flight_points.name=="climb" )& (flight_points.altitude<float(end_of_cruise.altitude))].iloc[-1])
        # Set OpenMDAO outputs
        # outputs["data:mission:"+mission_type+":initial_climb:fuel"] = (
        #     end_of_initial_climb.fuel_mass
        # )
        outputs["data:mission:"+mission_type+":cruise:max_mach"] = end_of_climb.max_mach
        ...
        outputs["data:mission:"+mission_type+":climb:operational_ceiling"] = top_of_climb.ceiling
        if end_of_cruise.altitude < cruise_altitude*foot:
            outputs["data:mission:"+mission_type+":climb:operational_ceiling"] = end_of_cruise.altitude
        ##########################
        ...
        if 'FC' in self.options["propulsion_id"] or 'H2' in self.options["propulsion_id"]:

            outputs["data:mission:"+mission_type+":initial_climb:H2"] = (
        ...
        outputs["data:mission:"+mission_type+":main_route:climb:TTC"] = top_of_climb.time - end_of_initial_climb.time
        ...
        diversion_electric_power_rates = {
            FlightPhase.CLIMB: inputs["data:mission:"+mission_type+":diversion:climb:EM_power_rate"],
            FlightPhase.CRUISE: inputs["data:mission:"+mission_type+":diversion:cruise:EM_power_rate"],
        }     
        
        diversion_turbine_power_rates = {
            FlightPhase.CLIMB: inputs["data:mission:"+mission_type+":diversion:climb:TP_power_rate"],
            FlightPhase.CRUISE: inputs["data:mission:"+mission_type+":diversion:cruise:TP_power_rate"],
        }         
        if 'FHFC' in self.options["propulsion_id"] or 'H2' in self.options["propulsion_id"]:
        ...
        if diversion_distance>0:
            ...
            delta_time=[diversion_flight_points.time.iloc[1]-diversion_flight_points.time.iloc[0]] +[diversion_flight_points.time.iloc[i+1]-diversion_flight_points.time.iloc[i] for i in range(len(diversion_flight_points)-1)]
            diversion_flight_points['delta_time']=np.array(delta_time)    
            fuel_delta_mass = diversion_flight_points.psfc * diversion_flight_points.TPshaft_power *diversion_flight_points.delta_time
            diversion_flight_points['fuel_mass']= fuel_delta_mass.cumsum()
            
            if hybrid_diversion:
                H2_delta_mass = diversion_flight_points.delta_time*diversion_flight_points.H2_fc
                diversion_flight_points['H2_mass']= H2_delta_mass.cumsum()
            if 'EB' in self.options["propulsion_id"]:
                BAT_delta_energy_div = (diversion_flight_points.delta_time * diversion_flight_points.BAT_ec )  #W*s=J
            ...
            end_of_diversion_climb = FlightPoint(
                diversion_flight_points.loc[diversion_flight_points.name == "diversion climb"].iloc[-1]
            )
            end_of_diversion_cruise = FlightPoint(
                diversion_flight_points.loc[diversion_flight_points.name == "diversion cruise"].iloc[-1]
            )
            end_of_diversion_descent = FlightPoint(
                diversion_flight_points.loc[diversion_flight_points.name == "diversion descent"].iloc[-1]
            )
            top_of_diversion_climb = FlightPoint(diversion_flight_points[(diversion_flight_points.name=="diversion climb" )& (diversion_flight_points.altitude<float(end_of_diversion_cruise.altitude))].iloc[-1])
            ...
            outputs["data:mission:"+mission_type+":diversion:climb:operational_ceiling"] = top_of_diversion_climb.ceiling
            if end_of_diversion_cruise.altitude < diversion_cruise_altitude*foot:
                outputs["data:mission:"+mission_type+":diversion:climb:operational_ceiling"] = end_of_diversion_cruise.altitude
            # Set OpenMDAO outputs
            ...
            if hybrid_diversion:
                ...
                outputs["data:mission:"+mission_type+":diversion:climb:H2"] = (
            ...
            end_of_diversion_descent = FlightPoint(
                                ...
                                fuel_mass =0.,#inputs["data:mission:"+mission_type+":takeoff:fuel"],
        ...
        if holding_duration>0:
            ...
            end_of_diversion_descent.equivalent_airspeed = 80. 
            atmosphere = Atmosphere(end_of_diversion_descent.altitude, altitude_in_feet=False)
            v_tas_hold= atmosphere.get_true_airspeed(end_of_diversion_descent.equivalent_airspeed)
            ...
            #holding_flight_points = holding_calculator.compute_from(end_of_descent)
             ...
             #Complete flight_points with H2 and fuel cumulative masses
            delta_time=[holding_flight_points.time.iloc[1]-holding_flight_points.time.iloc[0]] +[holding_flight_points.time.iloc[i+1]-holding_flight_points.time.iloc[i] for i in range(len(holding_flight_points)-1)]
            holding_flight_points['delta_time']=np.array(delta_time)      
            fuel_delta_mass = holding_flight_points.psfc * holding_flight_points.TPshaft_power *holding_flight_points.delta_time
            holding_flight_points['fuel_mass']= fuel_delta_mass.cumsum()
            
            if hybrid_holding:
                H2_delta_mass = holding_flight_points.delta_time*holding_flight_points.H2_fc
            ...
            if hybrid_holding:            
                ...
                outputs["data:mission:"+mission_type+":holding:H2"] = (
        ...
        # Final ================================================================
        allowances = inputs["data:mission:"+mission_type+":takeoff:fuel"]+inputs["data:mission:"+mission_type+":taxi_out:fuel"]+ inputs["data:mission:"+mission_type+":landing:fuel"]+inputs["data:mission:"+mission_type+":taxi_in:fuel"]
        allowances_time = inputs["data:mission:"+mission_type+":takeoff:duration"]+inputs["data:mission:"+mission_type+":taxi_out:duration"]+ inputs["data:mission:"+mission_type+":landing:duration"]+inputs["data:mission:"+mission_type+":taxi_in:duration"]
        
        fuel_route = end_of_descent.fuel_mass
        H2_route = sum(filter(None, [end_of_holding.H2_mass,end_of_diversion_descent.H2_mass,end_of_descent.H2_mass]))
        ...
        allowances_H2 = 0.15*H2_route + inputs["data:mission:"+mission_type+":takeoff:H2"]+inputs["data:mission:"+mission_type+":taxi_out:H2"]+ inputs["data:mission:"+mission_type+":landing:H2"]+inputs["data:mission:"+mission_type+":taxi_in:H2"] #h2 to keep cryotemp at the end of the mission
        if inputs["data:mission:"+mission_type+":diversion:H2_allowances"]==1:
            diversion_allowances_H2 = outputs["data:mission:"+mission_type+":main_route:climb:H2"] #inputs["data:mission:"+mission_type+":diversion:H2_allowances"]
        else:
        ...
        outputs["data:mission:"+mission_type+":block_fuel"] = fuel_route + allowances
        outputs["data:mission:"+mission_type+":block_time"] =  end_of_descent.time -end_of_initial_climb.time +allowances_time
        # if mission_type=='sizing':
        #     NPAX = inputs["data:TLAR:NPAX"]
        #     outputs["data:mission:sizing:block_fuel_pax"] = outputs["data:mission:"+mission_type+":block_fuel"]/NPAX
        outputs["data:mission:"+mission_type+":ZFW"] = end_of_holding.mass- inputs["data:mission:"+mission_type+":landing:fuel"] - inputs["data:mission:"+mission_type+":taxi_in:fuel"] - 0.05 * fuel_route - allowances_H2 - diversion_allowances_H2
        outputs["data:mission:"+mission_type+":fuel"] = end_of_holding.fuel_mass+end_of_diversion_descent.fuel_mass+ end_of_descent.fuel_mass+ allowances + 0.05 * fuel_route
        ...
        if 'FC' in self.options["propulsion_id"] or 'H2' in self.options["propulsion_id"]:            
            ...
            outputs["data:mission:"+mission_type+":H2"] = H2_route + allowances_H2 + diversion_allowances_H2
            outputs["data:mission:"+mission_type+":trip_H2"]=end_of_descent.H2_mass #+ allowances_H2            
        elif 'EB' in self.options["propulsion_id"]:    
            ...
            self.flight_points = (
                pd.concat(
                    [   #take_off_flight_points.drop(columns=['TP_total_temperature','TP_total_pressure']),
                        flight_points.drop(columns=['TP_total_temperature','TP_total_pressure']),
                        diversion_flight_points.drop(columns=['TP_total_temperature','TP_total_pressure']),
                        holding_flight_points.drop(columns=['TP_total_temperature','TP_total_pressure']),
            ...
            thermo_data= (
                pd.concat(
                    [   #take_off_flight_points[['TP_total_temperature','TP_total_pressure']],
                        ...
                        diversion_flight_points[['TP_total_temperature','TP_total_pressure']],
                        holding_flight_points[['TP_total_temperature','TP_total_pressure']],
                        #taxi_in_flight_points,
                    ]
                )
                .reset_index(drop=True)
