...
from rhea.models.performances.mission.segments.ASDN1_flight import ASDN1Segment
...
AddKeyAttributes(["alpha", "max_mach", "DISA"])(FlightPoint)
...
class TakeOffFlight(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("data:mission:sizing:takeoff:TP_power_rate", 1.0)
        ...
        self.add_input("tuning:aerodynamics:aircraft:low_speed:CD:k", 1.0)

        # Outputs ----------------------------------------------------------------------------------
        self.add_output("data:mission:sizing:takeoff:TOFL", units="m", val=0.0)
        ...
        self.add_output("data:mission:sizing:takeoff:V_1", units="m/s", val=0.0)
        self.add_output("data:mission:sizing:takeoff:V_EF", units="m/s", val=0.0)
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        TOargs = {
            ...
            "kf_brake": inputs[
                "data:mission:sizing:takeoff:Friction_coefficient_brake"
            ],
        }

        k_cd = inputs["tuning:aerodynamics:aircraft:low_speed:CD:k"]

        take_off_polar = Polar(
            inputs["data:aerodynamics:aircraft:takeoff:CL"],
            inputs["data:aerodynamics:aircraft:takeoff:CD"] * k_cd,
        )
        ...
        initial_takeoff = FlightPoint(
            ...
            ground_distance=0.0,
            alpha=alpha_initial,
            EM_power_rate=inputs[
                "data:mission:" + mission_type + ":takeoff:EM_power_rate"
            ],
            TP_power_rate=inputs[
                "data:mission:" + mission_type + ":takeoff:TP_power_rate"
            ],
        ...
        def TOD():
            ...
            take_off_flight_points, vr, vlo, v2 = take_off_calculator.compute_from(
                initial_takeoff
            )
            ...
            try:
                delta_time = [
                    take_off_flight_points.time.iloc[1]
                    - take_off_flight_points.time.iloc[0]
                ] + [
                    take_off_flight_points.time.iloc[i + 1]
                    - take_off_flight_points.time.iloc[i]
                    for i in range(len(take_off_flight_points) - 1)
                ]
                take_off_flight_points["delta_time"] = np.array(
                    delta_time, dtype=object
                )
                fuel_delta_mass = (
                ...
                end_of_takeoff = FlightPoint(
                    take_off_flight_points.loc[
                        take_off_flight_points.name == "take off"
                    ].iloc[-1]
                )
            ...
            return (
                ...
                float(end_of_takeoff.ground_distance),
                float(outputs["data:mission:" + mission_type + ":takeoff:V_rotate"]),
            )
        ...
        def ASD(v1):
            ...
            end_of_ASD = FlightPoint(
                ASD_flight_points.loc[ASD_flight_points.name == "ASD"].iloc[-1]
            )
            ASD = end_of_ASD.ground_distance + v1 * 2  # +2sec at v1
            # print('ASD',float(ASD) )
        ...
        def ASDN1(vef):
            ...
            ASDN1_flight_points = ASDN1_calculator.compute_from(initial_takeoff)
            # print('     ASD completed')
            # flight_point_vef = ASD_flight_points.loc[ASD_flight_points.equivalent_airspeed == TOargs['vef']]
            time_v1 = float(
                ASDN1_flight_points[ASDN1_flight_points.engine_setting == 8][
                    "time"
                ].iloc[0]
                + 1
            )
            flight_point_v1 = ASDN1_flight_points.loc[
                ASDN1_flight_points.time >= time_v1
            ].iloc[0]
            end_of_ASDN1 = FlightPoint(
                ASDN1_flight_points.loc[ASDN1_flight_points.name == "ASDn-1"].iloc[-1]
            )
            ASDN1 = (
                end_of_ASDN1.ground_distance + flight_point_v1.equivalent_airspeed * 2
            )  # +2sec at v1
            # print('ASD',float(ASD) )
            # NB: there is no model thrust-power for flight idle, so in the braking segment the power
            #    calculated is not to be taken into account. Anyway, only the thrust has an impact on the performance!
            ...
            return ASDN1_flight_points, float(ASDN1)

        TOD_flight_points, tod_n, vr = TOD()  # TODn
        TOD_flight_points["name"] = "TOD"
        ...
        if vr > 0:
            ...
            def func(vef):
                ...
                ASDN1_flight_points, asd_n1 = ASDN1(vef)
                time_v1 = float(
                    ASDN1_flight_points[ASDN1_flight_points.engine_setting == 8][
                        "time"
                    ].iloc[0]
                ...
                ASD_flight_points, asd = ASD(flight_point_v1.equivalent_airspeed[0])
                return tod - max(asd, asd_n1)

            vef, info, ier, msg = fsolve(
                func, vr, full_output=True, xtol=10e-4
            )  # ,epsfcn=10e2)
            ...
            if flight_point_v1.equivalent_airspeed[0] > vr:
                vef = vr - (
                    flight_point_v1.equivalent_airspeed[0] - vef
                )  # vef for which v1=vr (approximately)
                take_off_flight_points, tod_n1, vr = TODn1(vef)
                print("TODn-1 = ", tod_n1)
                ASDN1_flight_points, asd_n1 = ASDN1(vef)
                print("ASDn-1 = ", asd_n1)
                ASD_flight_points, asd = ASD(vr)
                print("ASD = ", asd)
                BFL = max([float(tod_n), float(tod_n1), float(asd_n1), float(asd)])
                print("V1>VR")
                v1 = vr
            else:
                print("V1<VR")
        ...
        outputs["data:mission:sizing:takeoff:distance"] = tod_n
