...
class TakeOffFlight(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("tuning:aerodynamics:aircraft:low_speed:CD:k", 1.)
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        def ASDN1(vef):
            ...
            **TOargs,
            ...
            time_v1 = float(ASDN1_flight_points[ASDN1_flight_points.engine_setting == 8]['time'].iloc[0]+1)
            flight_point_v1 = ASDN1_flight_points.loc[ASDN1_flight_points.time >= time_v1].iloc[0]
            end_of_ASDN1 = FlightPoint(ASDN1_flight_points.loc[ASDN1_flight_points.name == "ASDn-1"].iloc[-1])
            ASDN1 = end_of_ASDN1.ground_distance + flight_point_v1.equivalent_airspeed*2  #+2sec at v1
            # print('ASD',float(ASD) ) 
            #NB: there is no model thrust-power for flight idle, so in the braking segment the power 
            #    calculated is not to be taken into account. Anyway, only the thrust has an impact on the performance!
            #    (segment has thrust_rate)
