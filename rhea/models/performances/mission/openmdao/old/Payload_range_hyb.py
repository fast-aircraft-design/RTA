...
class Payload_range(SizingFlight_RHEA):
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        PL_A, Range_A, fuel_A, TOW_A, ZFW_A, H2_A = self.compute_point_A(
            inputs, outputs
        )
        PL_B, Range_B, fuel_B, TOW_B, ZFW_B, H2_B = self.compute_point_B(
            inputs, outputs
        )

        PL_C, Range_C, fuel_C, TOW_C, ZFW_C, H2_C = self.compute_point_C(
            inputs, outputs
        )
        PL_D, Range_D, fuel_D, TOW_D, ZFW_D, H2_D = self.compute_point_D(
            inputs, outputs
        )
        ...
        H2_list = np.array([0.0, 0.0, H2_A, H2_B, H2_C, H2_D])
