...
class SizingFlight_RHEA(om.ExplicitComponent):
    ...
    def compute_mission(self, inputs, outputs,loop_inputs,mission_type):
        ...
        # OEI ceiling flight =====================================================
        if mission_type=='sizing':
            altitude_OEI=inputs["data:mission:"+mission_type+":takeoff:altitude"] + 1500 * foot
            density_OEI= Atmosphere(altitude_OEI, altitude_in_feet=False).density
            
            VSR = (2 * end_of_takeoff.mass * g / (inputs["data:aerodynamics:aircraft:takeoff:CL_max"] * reference_area * density_OEI))**0.5
            print(end_of_takeoff.true_airspeed ,1.18*VSR)

            init_OEI_climb = FlightPoint(
                mass=TOW_input- inputs["data:mission:"+mission_type+":takeoff:fuel"]-inputs["data:mission:"+mission_type+":taxi_out:fuel"],
            ...
            low_speed_climb_polar_OEI = Polar(
                inputs["data:aerodynamics:aircraft:low_speed:CL"],
                inputs["data:aerodynamics:aircraft:low_speed:CD"],#+0.0046,#+0.002,   #0.002for AIAA study
            )        
            ...
            try:
                flight_point_ceiling = OEI_flight_points[OEI_flight_points.RC_penalty<0.254].iloc[0] #0.254m/s=50 ft/min
        ...
        top_of_climb = FlightPoint(flight_points[(flight_points.name=="climb" )& (flight_points.altitude<=float(cruise_altitude*foot))].iloc[-1])
        ...
        outputs["data:mission:"+mission_type+":ZFW"] = end_of_holding.mass- inputs["data:mission:"+mission_type+":landing:fuel"] - inputs["data:mission:"+mission_type+":taxi_in:fuel"] - 0.05 * fuel_route - allowances_H2
