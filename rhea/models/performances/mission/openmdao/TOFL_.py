...
class TakeOffFlight(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        def ASD(vef):
            ...
            time_v1 = float(
                ASD_flight_points[ASD_flight_points.engine_setting == 8]["time"].iloc[0]
                + 1
            )
            flight_point_v1 = ASD_flight_points.loc[
                ASD_flight_points.time >= time_v1
            ].iloc[0]
            end_of_ASD = FlightPoint(
                ASD_flight_points.loc[ASD_flight_points.name == "ASD"].iloc[-1]
            )
            ASD = (
                end_of_ASD.ground_distance + flight_point_v1.equivalent_airspeed * 2
            )  # +2sec at v1
            # print('ASD',float(ASD) )
        ...
        if vr > 0:
            ...
            Tot_av_power = (
                propulsion_model.engine.Elec_nom_power
                * inputs["data:mission:" + mission_type + ":takeoff:EM_power_rate"]
                + propulsion_model.engine.RTO_power
                * inputs["data:mission:" + mission_type + ":takeoff:TP_power_rate"]
            )
            ...
            f = interpolate.interp1d(
                SHP_TOs, TOFLs, kind="linear", fill_value="extrapolate"
            )
            BFL = f(Tot_av_power)
            vef = 0
            v1 = 0
        ...
        try:
            if (
                ...
                self.flight_points = (
                    pd.concat([take_off_flight_points, ASD_flight_points])
                    .reset_index(drop=True)
                    .applymap(lambda x: np.asscalar(np.asarray(x)))
                )
