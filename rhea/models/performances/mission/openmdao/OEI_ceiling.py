...
class OEI_ceiling(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("data:aerodynamics:aircraft:low_speed:OEI_effect:DCD", np.nan,copy_shape="data:aerodynamics:aircraft:low_speed:CT")
        ...
        # Outputs ----------------------------------------------------------------------------------
        self.add_output("data:mission:sizing:OEI:net_ceiling",  units="m",val=0.)
        self.add_output("data:mission:sizing:OEI:V_TAS",  units="m/s",val=0.)
        self.add_output("data:mission:sizing:OEI:mass", units="kg",val=0.)
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        self.em_power_rate = min(inputs["data:mission:sizing:climb:EM_power_rate"].min()*2 , 1)
        # EM_power_rate = 
        ...
        def func(alt,m):
            ...
            V_TAS =self.find_best_climb_speed(high_speed_polar,reference_area,atmosphere,weight)
            
            
            mach = V_TAS/atmosphere.speed_of_sound
            CL= weight/(0.5*density*reference_area*V_TAS**2)
            FN= self.evaluate_thrust(propulsion_model_OEI.engine,atmosphere, mach)#*1.04 #altitude in feet    
            ...
            return ceiling
        ceiling = fsolve(func, 10000.,args=mass)#inputs["data:weight:aircraft:MTOW"])
        V_TAS = self.find_best_climb_speed(high_speed_polar,reference_area,Atmosphere(ceiling, altitude_in_feet=True),inputs["data:weight:aircraft:MTOW"]*9.81)
    ...
    def evaluate_thrust(self,propulsion_model_OEI,atmosphere, mach):        
        ...
        if 'PH'  in self.options["propulsion_id"] or 'FH' in self.options["propulsion_id"]:               
            ...
            motor_power =propulsion_model_OEI.Elec_nom_power * self.em_power_rate  #-->diversion climb is the sizing point(No H2 available)
            propulsion_model_OEI = propulsion_model_OEI.turbine
        else:
            motor_power=0
        
        max_shaft_power,max_thermo_power,gearbox_limit_power   = propulsion_model_OEI.max_power(atmosphere, mach, phase)
        tot_shaft_power = max_shaft_power + motor_power
        max_thrust, eta =Propeller().select('power_to_thrust',self.options["prop_fid"],propulsion_model_OEI,atmosphere, mach,phase, tot_shaft_power)
        FR = propulsion_model_OEI.compute_engine_point(atmosphere, mach, phase, T_prop=max_thrust, shaft_power=max_shaft_power) 
    ...
    def find_best_climb_speed(self,high_speed_polar,reference_area,atmosphere,weight):
