...
class FinalTakeoffSegment(InitialTakeoffSegment):
    ...
    def complete_flight_point(self, flight_point: FlightPoint):
        ...
        flight_point.slope_angle, flight_point.acceleration = self._get_gamma_and_acceleration(
            ...
            flight_point.alpha, flight_point.true_airspeed)
    ...
    def _get_distance_to_target(self, flight_points: List[FlightPoint]) -> bool:
        self.target.slope_angle = flight_points[-1].slope_angle #just a way to store previous slope_angle        
              
        if self.target.altitude:
            return self.target.altitude - flight_points[-1].altitude


    def _get_gamma_and_acceleration(self, mass, drag,lift, thrust,alpha,V) -> Tuple[float, float]:
        if self.target.slope_angle:
            pass
        else:
            self.target.slope_angle=0
        # print('previous gamma',self.target.slope_angle) 
        d_gamma = (thrust * math.sin(alpha) + lift - mass*g * math.cos(self.target.slope_angle)) / (mass * V)
        # print('d_gamma',d_gamma) 
        if d_gamma <= 0:
            d_gamma = 0

        acceleration = ( thrust * math.cos(alpha) - drag - mass*g  * math.sin(self.target.slope_angle)) / mass
