...
class TakeOffSpeeds(TakeOffSegment):
    ...
    def compute_from(self, start: FlightPoint) -> pd.DataFrame:
        ...
        print('speeds found')
        self.segment_kwargs['vef']=vef  #reset engine failure speed value for actual TOFL calculation
        if self.v2>0:
    ...
    def find_speeds(self,start):
        ...
        flight_point_v2.altitude = start.altitude + 35.* foot
        ...
        def find_v2(factor):
            ...
            if v2_min == 1.1*self.vmca:                
                Cl = 2 * flight_point_v2.mass * g/(v2_min**2* S_ref * atm.density)
                print('v2_min = 1.1*vmca',Cl)
            
            flight_point_v2.true_airspeed = v2_min
            flight_point_v2.CL = Cl
            ...
            segment.propulsion.engine_count=1.#OEI
            segment.propulsion.motor_count=1.
            ...
            CT= thrust_OEI/(0.5* S_ref * atm.density*flight_point_v2.true_airspeed**2)
            flight_point_v2.CT = CT
            DCd_OEI = np.interp(flight_point_v2.CT, self.segment_kwargs['CT_list'], self.segment_kwargs['DCd_OEI'])
            Cd = segment.polar.cd(Cl) + DCd_OEI #negleting effects (because we don't have alpha at this point) but also reguulations requirements impose not to consider ground effect and gear are retracted (so no lg effect)
            climb_rate = (thrust_OEI / (flight_point_v2.mass * g ) - Cd / Cl) * 100 #=tan(gamma)*100
            
            segment.propulsion.engine_count=2 #reset AEO condition for next calculations
            segment.propulsion.motor_count=2 #reset AEO condition for next calculations
        ...
        if self.v2_min>0:# and Tot_av_power>1812000:
            ...
            while True:
                ...
                while True:   
                    ...
                    flight_points_Inter['lift_off'] = (-flight_points_Inter.mass*g - 
                                                       flight_points_Inter.drag*np.sin(flight_points_Inter.alpha.astype(float)) + 
                                                       (flight_points_Inter.CL*0.5*rho*flight_points_Inter.true_airspeed**2*S_ref)*np.cos(flight_points_Inter.alpha.astype(float)) + 
                                                       flight_points_Inter.thrust*np.sin(flight_points_Inter.alpha.astype(float)) )
                   
                    #find the flght_point which corresponds to vlo
                    try:
                        flight_point_vlo = flight_points_Inter[flight_points_Inter.lift_off>0].iloc[0]
                ...
                if self.vlo >self.v2_min:  
                    ...
                    if v2>self.v2_min:
                        ...
                        # print('V2',v2)
                        #print('actual v2 is lower than v2_min: Vr is increased')
                        VR += 0.5
            ...
            self.vr = 0
            self.v2 = 0
            print('Warning: Take-off cannot be performed with available TO power',Tot_av_power/1000, 'KW' )
