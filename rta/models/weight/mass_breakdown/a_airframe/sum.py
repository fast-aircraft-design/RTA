...
class AirframeWeight(om.Group):
    ...
    def setup(self):
        # Airframe
        self.add_subsystem("loads", RegisterSubmodel.get_submodel(SERVICE_GUST_LOADS), promotes=["*"])
        self.add_subsystem("ATA57", RegisterSubmodel.get_submodel(SERVICE_WING_MASS), promotes=["*"])
        self.add_subsystem("ATA53", RegisterSubmodel.get_submodel(SERVICE_FUSELAGE_MASS), promotes=["*"])
        self.add_subsystem("ATA55", RegisterSubmodel.get_submodel(SERVICE_EMPENNAGE_MASS), promotes=["*"])
        self.add_subsystem("ATA32", RegisterSubmodel.get_submodel(SERVICE_LANDING_GEARS_MASS), promotes=["*"])
        self.add_subsystem("ATA54", RegisterSubmodel.get_submodel(SERVICE_NACELLE_MASS), promotes=["*"])
